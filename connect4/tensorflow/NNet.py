...
from .Connect4NNet import ResNet as onnet
...
class NNetWrapper(NeuralNet):
    ...
    def train(self, examples):
        ...
        for epoch in range(args.epochs):
            ...
            while batch_idx < int(len(examples) / args.batch_size):
                ...
                #_ = self.sess.run([self.nnet.x], feed_dict=input_dict)
    ...
    def save_checkpoint(self, folder='checkpoint', filename='checkpoint.pth.tar'):
        folder1 = folder + "connect4/"
        filepath = os.path.join(folder1, filename)
        if not os.path.exists(folder1):
            print("Checkpoint Directory does not exist! Making directory {}".format(folder1))
            os.mkdir(folder1)
        else:
