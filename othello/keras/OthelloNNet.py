...
class OthelloNNet():
    def __init__(self, game, args):
        ...
        x = Conv2D(filters=256, kernel_size=3, strides=1, padding="same", data_format="channels_first")(x_image)
    ...
    def _build_residual_block(self, x, index):
        in_x = x
        x = Conv2D(filters=256, kernel_size=3, strides=1, padding="same", data_format="channels_first")(x)
        x = BatchNormalization(axis=1)(x)
        x = Activation("relu")(x)
        x = Conv2D(filters=256, kernel_size=3, strides=1, padding="same", data_format="channels_first")(x)
        x = BatchNormalization(axis=1)(x)
